drop database if exists LibraryDB;                     #nho xoa di nka
CREATE DATABASE IF NOT EXISTS LibraryDB;
USE LibraryDB;

CREATE TABLE users (
    user_id INT PRIMARY KEY AUTO_INCREMENT,
	username VARCHAR(100) NOT NULL,
    email VARCHAR(100) UNIQUE NOT NULL,
    password VARCHAR(100) NOT NULL,
    role VARCHAR(20) NOT NULL
);
										     #cai nay se dung cho add sach thui
CREATE TABLE books (
    title VARCHAR(255) NOT NULL,
    author VARCHAR(100),
    publisher VARCHAR(100) NOT NULL,
    publishedDate VARCHAR(50),
    isbn VARCHAR(20) NOT NULL PRIMARY KEY UNIQUE,
    thumbnail VARCHAR(10000),
    #status ENUM('AVAILABLE', 'BORROWED', 'DAMAGED', 'LOST') DEFAULT 'AVAILABLE', toi tam thoi se de z
    total_number INT DEFAULT 1,
    readed int default 0
);
#toi ko biet nhe
CREATE TABLE transactions (
    transaction_id INT PRIMARY KEY AUTO_INCREMENT,
    user_id INT,
    isbn VARCHAR(20),
    borrow_date DATETIME,
    due_date DATETIME,
    return_date DATETIME,
    fee DOUBLE default 0,
    FOREIGN KEY (user_id) REFERENCES users(user_id),
    FOREIGN KEY (isbn) REFERENCES books(isbn)
);
insert into users (username,email,password,role) values ('maomao','maomao@email.com','maomaone','admin');
insert into users (username,email,password,role) values ('1','1@email.com','1','customer');

insert into books (title,author,publisher,publishedDate,thumbnail,isbn,readed) values ("blacktuyet","woker","hoolishit","2024-10-16","https://kenh14cdn.com/thumb_w/620/203336854389633024/2022/9/17/photo-12-1663394501769503174426.jpg","12345",1);
insert into books (title,publisher,publishedDate,thumbnail,isbn,readed)
 values ("DiddyVsKitty","hoolishit","2024-10-16","https://hips.hearstapps.com/hmg-prod/images/sean-diddy-combs-on-friday-september-15-2023-news-photo-1711468298.jpg",'1234566',3);
#insert into books (title,author,publisher,publishedDate,thumbnail,isbn,readed) values ("maomao","kitty","mangaku","2024-10-18","https://i.etsystatic.com/20736936/r/il/fe7f4a/5631396316/il_570xN.5631396316_gxi8.jpg","123",2);
insert into books (title,author,publisher,publishedDate,thumbnail,isbn,readed) values ("maomao","kitty","mangaku","2024-10-18","data:image/jpeg;base64,","123",2);
insert into transactions (user_id,isbn,borrow_date,due_date) values (1,'12345','2024-10-16','2024-12-31');


